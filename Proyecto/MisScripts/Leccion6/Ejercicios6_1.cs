...
public class Ejercicios6_1 : MonoBehaviour
{
	...
	void Start(){
		...
		else if(puntuacion==140){
			i=2;
		}
		else if(puntuacion==145){
			i=3;
		}
		else{
			repetido=true;
		...
		Social.ReportScore(0,leaderboard,(bool success) =>{});
	...
	void Update(){
		...
		if(!saveActivo){
			...
			if(intentos==0){
				if(!repetido){
					SaveLoad save=new SaveLoad(125,3,0);
					save.Save();
				}
		...
		if(correcto){
			...
			if(i==3){
				...
				SCORE.GetComponent<TextMesh>().text = puntuacion.ToString();
				if(!repetido){
					SaveLoad save=new SaveLoad(puntuacion,intentos,Mathf.RoundToInt(reloj),6,Mathf.RoundToInt(reloj));
					save.Save();
				}
				correcto=false;
