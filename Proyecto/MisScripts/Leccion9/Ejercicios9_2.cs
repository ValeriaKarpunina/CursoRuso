...
public class Ejercicios9_2 : MonoBehaviour
{
	...
	int i;
	int puntuacion;
	GameObject palabra1,palabra2,palabra3,palabra4,palabra5,eleccion1,eleccion2,eleccion3, SCORE, Correcto, Incorrecto,
	...
	void Start(){
		...
		else if(puntuacion==210){
			i=2;
		}
		else if(puntuacion==215){
			i=3;
		}
		else if(puntuacion==220){
			i=4;
		}
		else if(puntuacion==225){
			i=5;
		}
		else{
			repetido=true;
	...
	void Update(){
		...
		if(correcto){
			...
			if(i==5){
				...
				if(!repetido){
					SaveLoad save=new SaveLoad(puntuacion,intentos,Mathf.RoundToInt(reloj),9,Mathf.RoundToInt(reloj));
					save.Save();
				}
				correcto=false;
