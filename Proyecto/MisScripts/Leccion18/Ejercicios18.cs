...
public class Ejercicios18 : MonoBehaviour
{
	...
	void Update(){
		...
		if(!saveActivo){
			...
			if(intentos==0){
				if(!repetido){
					SaveLoad save=new SaveLoad(415,3,0);
					save.Save();
				}
		...
		if(correcto){
			...
			if(i==2){
				...
				SCORE.GetComponent<TextMesh>().text = puntuacion.ToString();
				if(!repetido){
					SaveLoad save=new SaveLoad(puntuacion,intentos,Mathf.RoundToInt(reloj),18,Mathf.RoundToInt(reloj));
					save.Save();
				}
				correcto=false;
