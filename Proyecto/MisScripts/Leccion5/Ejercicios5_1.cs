...
public class Ejercicios5_1 : MonoBehaviour
{
	...
	void Update(){
		...
		if(!saveActivo){
			...
			if(intentos==0){
				if(!repetido){
					SaveLoad save=new SaveLoad(100,3,0);
					save.Save();
				}
		...
		if(correcto){
			...
			if(i==3){
				...
				SCORE.GetComponent<TextMesh>().text = puntuacion.ToString();
				if(!repetido){
					SaveLoad save=new SaveLoad(puntuacion,intentos,Mathf.RoundToInt(reloj),5,Mathf.RoundToInt(reloj));
					save.Save();
				}
				correcto=false;
