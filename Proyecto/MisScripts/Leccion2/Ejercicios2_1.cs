...
public class Ejercicios2_1 : MonoBehaviour
{
	...
	void Start(){
		...
		//SaveLoad save=new SaveLoad(1,2,13,3,39);
		//save.Save();

		//ClienteTCP2 cliente=new ClienteTCP2(3);
		//for(int i=0;i<3;i++)
		//	Debug.Log("DATOS: "+cliente.MisDatos()[i]);

		//PonerDatos(3,cliente.GetName1(),cliente.GetName2(),cliente.GetName3(),cliente.GetName4(),cliente.GetInt());
	}
	...
	void Update(){
		...
		if(!saveActivo){
			...
			if(intentos==0){
				...
				if(!repetido){
					SaveLoad save=new SaveLoad(20,3,0);
					save.Save();
				}
			...
			if(correcto){
				...
				if(j==1){
					...
					if(!repetido){
						SaveLoad save=new SaveLoad(puntuacion,intentos,Mathf.RoundToInt(reloj),2,Mathf.RoundToInt(reloj));
						save.Save();
					}
