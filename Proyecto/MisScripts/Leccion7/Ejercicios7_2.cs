...
public class Ejercicios7_2 : MonoBehaviour
{
	...
	float tiempo;
	float reloj;
	GameObject Ya, Student,Est,Byt, Nada, Score,Menya, Nombre,zovut,tebya, imya, Corregir,
	...
	void Update(){
			...
			if (Physics.Raycast(ray, out hit))
			{
					if (theTouch.phase == TouchPhase.Began)
					{
						switch(hit.transform.name){
							...
							if(zovut.transform.position.x < 520){
							...
							if(Nada.transform.position.x < 520){
									Correcto.SetActive(true);
									Incorrecto.SetActive(false);
									correcto=true;
		...
		if(i==2){
			if(correcto){
				...
				if(!repetido){
					SaveLoad save=new SaveLoad(puntuacion,intentos,Mathf.RoundToInt(reloj),7,Mathf.RoundToInt(reloj));
					save.Save();
				}
		...
		if(!saveActivo){
			...
			if(intentos==0){
				if(!repetido){
					SaveLoad save=new SaveLoad(150,3,0);
					save.Save();
				}
