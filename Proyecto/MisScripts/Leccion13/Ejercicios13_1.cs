...
public class Ejercicios13_1 : MonoBehaviour
{
	...
	int i;
	int puntuacion;
	GameObject palabra1,palabra2,palabra3,palabra4,palabra5,eleccion1,eleccion2,eleccion3,eleccion4, SCORE, Correcto, Incorrecto,
	...
	void Start(){
		...
		else if(puntuacion==325){
			i=2;
		}
		else if(puntuacion==330){
			i=3;
		}
		else if(puntuacion==335){
			i=4;
		}
		else if(puntuacion==340){
			i=5;
		}
		else{
			repetido=true;
	...
	void Update(){
		...
		if(!saveActivo){
			...
			if(intentos==0){
				if(!repetido){
					SaveLoad save=new SaveLoad(310,3,0);
					save.Save();
				}
		...
		if(correcto){
			...
			if(i==5){
				...
				SCORE.GetComponent<TextMesh>().text = puntuacion.ToString();
				if(!repetido){
					SaveLoad save=new SaveLoad(puntuacion,intentos,Mathf.RoundToInt(reloj),13,Mathf.RoundToInt(reloj));
					save.Save();
				}
				correcto=false;
