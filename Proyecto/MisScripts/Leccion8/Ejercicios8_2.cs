...
public class Ejercicios8_2 : MonoBehaviour
{
	int i,k; 
	...
	void Update(){
		...
		if(i==2){
			if(correcto){
				...
				SCORE.GetComponent<TextMesh>().text = puntuacion.ToString();
				if(!repetido){
					SaveLoad save=new SaveLoad(puntuacion,intentos,Mathf.RoundToInt(reloj),8,Mathf.RoundToInt(reloj));
					save.Save();
				}
