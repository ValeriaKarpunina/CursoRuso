...
public class Ejercicios17 : MonoBehaviour
{
	...
	int i;
	int puntuacion;
	GameObject palabra1,palabra2,palabra3,palabra4,palabra5,eleccion1,eleccion2,eleccion1_2,eleccion2_2,eleccion1_3,eleccion2_3,eleccion3,eleccion4, SCORE, Correcto, Incorrecto,
	...
	void Start(){
		...
		else if(puntuacion==405){
			i=2;
			reloj=valores[2];
			intentos=valores[1];
		}
		else if(puntuacion==410){
			i=3;
			reloj=valores[2];
			intentos=valores[1];
		}
	...
	void Update(){
		...
		if(!saveActivo){
			...
			if(intentos==0){
				if(!repetido){
					SaveLoad save=new SaveLoad(400,3,0);
					save.Save();
				}
		...
		if(correcto){
			...
			if(i==3){
				...
				if(!repetido){
					SaveLoad save=new SaveLoad(puntuacion,intentos,Mathf.RoundToInt(reloj),17,Mathf.RoundToInt(reloj));
					save.Save();
				}
				correcto=false;
