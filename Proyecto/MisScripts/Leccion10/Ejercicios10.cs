...
public class Ejercicios10 : MonoBehaviour
{
	...
	int i;
	int puntuacion;
	GameObject palabra1,palabra2,palabra3,eleccion1,eleccion2,eleccion3,eleccion4, SCORE, Correcto, Incorrecto,
	...
	void Update(){
		...
		if(!saveActivo){
			...
			if(intentos==0){
				if(!repetido){
					SaveLoad save=new SaveLoad(230,3,0);
					save.Save();
				}
		...
		if(correcto){
			...
			if(i==3){
				...
				SCORE.GetComponent<TextMesh>().text = puntuacion.ToString();
				if(!repetido){
					SaveLoad save=new SaveLoad(puntuacion,intentos,Mathf.RoundToInt(reloj),10,Mathf.RoundToInt(reloj));
					save.Save();
				}
				correcto=false;
